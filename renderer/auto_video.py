...
# --- CONFIG ------------------------------------------------------------------
TARGET_DURATION   = USER_DURATION or _int("MANIMAX_TARGET_DURATION",  45, lo=10)
NUM_CHAPTERS      = _int("MANIMAX_NUM_CHAPTERS",       3, lo=1, hi=10)
CHAPTER_DURATION  = TARGET_DURATION // max(1, NUM_CHAPTERS)
RENDER_QUALITY    = os.getenv("MANIMAX_RENDER_QUALITY", "high").strip().lower()
TARGET_FPS        = _int("MANIMAX_TARGET_FPS", 60, lo=1)
SYNC_TOLERANCE    = _float("MANIMAX_SYNC_TOLERANCE", 0.2, lo=0.0)
ALLOW_LQ_FALLBACK = os.getenv("MANIMAX_ALLOW_LOW_QUALITY_FALLBACK","1").lower() in {"1","true","yes"}
TTS_VOICE         = os.getenv("MANIMAX_TTS_VOICE", "en-GB-RyanNeural")
...
AI_RETRY_DELAY    = _float("MANIMAX_AI_RETRY_DELAY_SECONDS", 3.0, lo=0)
AI_BACKOFF        = _int("MANIMAX_AI_TIMEOUT_BACKOFF_SECONDS", 30, lo=0)
