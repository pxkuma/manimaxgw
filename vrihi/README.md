# Vrihi — AI Video Generator (Manim + Edge-TTS + Gemma 4)

This project provides a fully automated pipeline for generating highly educational, aesthetic, YouTube-style technical explainer clips. By leveraging the **Gemma 4 (31B Cloud)** model via **Ollama**, the `edge-tts` speech synthesizer, and the **Manim** animation engine, it builds cinematic chapter-based video explanations from just a single topic prompt.
...
**Pipeline Breakdown:**
1. **Prompt Enhancement & Curriculum Planning:** The user's raw topic is sent to Gemma 4 via Ollama, which acts as a curriculum designer — breaking the topic into 6-10 detailed chapters with visual plans, real-world examples, coordinates, and audio narration blueprints.
2. **Script & Code Generation:** For each chapter, Gemma 4 generates comprehensive educational narration (10-15 sentences with examples) AND the actual Manim visualization code (strictly formatted via custom guardrails for safe rendering).
...
```bash
.
├── auto_video.py        # Main AI generator script (Gemma 4 + Ollama)
...
- **Ollama** installed natively on your machine (not in Docker)
...
# 2. Pull the Gemma 4 model (first time only)
ollama pull gemma4:31b-cloud

# 3. Build and start the Vrihi container
docker compose build
docker compose up -d

# 4. Run the interactive pipeline
docker compose exec vrihi bash vrihi.sh
```
...
# Or directly:
docker compose exec vrihi python auto_video.py "Docker Basics"
```
...
docker compose exec vrihi python autorun.py examples/sin.py
```
...
### Linux / macOS
```bash
# Double-click vrihi.sh or run:
./vrihi.sh
```

### Windows
```cmd
# Double-click vrihi.bat or run:
vrihi.bat
```
...
3. Run `ollama pull gemma4:31b-cloud` in Command Prompt
4. Double-click `vrihi.bat` to start
...
| `OLLAMA_MODEL` | `gemma4:31b-cloud` | Model to use for generation |
